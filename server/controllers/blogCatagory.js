...
const updateCategory = asyncHandler(async(req, res)=>{
    ...
    const response = await BlogCategory.findByIdAndUpdate(bcid,req.body,{new: true}).select('_id title')
    return res.status(200).json({
        success: response ? true : false,
...
const deleteCategory = asyncHandler(async(req, res)=>{
    ...
    const response = await BlogCategory.findByIdAndDelete(bcid).select('_id title')
    return res.status(200).json({
        success: response ? true : false,
