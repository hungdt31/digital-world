...
const updateCoupon = asyncHandler(async (req, res) => {
    ...
    return res.status(200).json({
        success: response ? true : false,
        createdCoupon: response ? response : "Cannot update coupon",
    });
});
const deleteCoupon = asyncHandler(async (req, res) => {
    const { cid } = req.params;
    const response = await Coupon.findByIdAndDelete(cid).select('-createdAt -updatedAt');
    return res.status(200).json({
        success: response ? true : false,
        deletedCoupon: response || "Something went wrong",
    });
});
module.exports = {
    createNewCoupon,
